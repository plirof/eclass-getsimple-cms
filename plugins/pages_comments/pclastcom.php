<?php
    ...
    if (@$_GET['dateseg'] != ''){
 	  ...
 	  echo '<span style="font-size: 20px; text-align: center; text-decoration: blink;"><b>--- '.i18n_r('pages_comments/pc_delreg').' ---</b></span><br /><br /><br /><br />';

?>
            <script type="text/javascript">
                    //setTimeout(function(){window.location="load.php?id=pages_comments&action=pc_last"},2000); //ORIG
                    setTimeout(function(){window.location="load.php?id=pages_comments&action=pc_viewpages"},2000);
...
if(file_exists($log_file)) {
      ...
      for ($q= $num; $q>=($num-$rctpost+1) ; $q--){
		...
		$title = substr($pc_url, strlen($pcpageNM),-4).'</span><span style="color:#777777;"> ('.i18n_r('pages_comments/pc_postsNM').')';
	  
	   }


           echo '<span style="color: #000000; text-align: center; font-family: Georgia,Times,Times New Roman,serif; font-size: 16px; font-weight: normal;">'.i18n_r('pages_comments/pc_pgttl').': '.$title.'</span>';
          ...
          foreach($log_data as $node) {
        	...
        	echo '<table style="line-height: 11px; margin-bottom: 0px; width: 100%;">';
         	    echo '<tr style="border-bottom: none;">';
         	          echo '<td style="border-bottom: none;width: 45px; padding: 0px; line-height: 11px;"><b>Atributo Id: </b>'.$atrib.'</td>';
			  ...
			  echo '<td style="border-bottom: none;width: 45px; padding: 0px; line-height: 11px;"><a style="padding-left: 25px;" title="'.i18n_r('pages_comments/ndel').'" onClick= "return confirm(&quot;'.i18n_r('pages_comments/ndel').'&quot;)" href="load.php?id=pages_comments&action=pc_last&pc_url='.$pc_url.'&dateseg='.$log_time.'">'.i18n_r('pages_comments/ndel').'</a></td>';
                    echo '</tr>';
        	echo '</table>';
